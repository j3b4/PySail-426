...
'''
#release 02: risolto bug sulla sezione 4 per dati in gruppi di 8 bit o multipli
...
class Grib:
    ...
    def leggirecord(self,file):
        ...
        bit18=file.read(1)#bit 18
        bit19=file.read(1)#bit 19
        bit20=file.read(1)#bit 20
        bit21=file.read(1)#bit 21
        record.timeunits=struct.unpack("1B",bit18)[0]
        record.P1=struct.unpack("1B",bit19)[0]#atteso 0
        record.P2=struct.unpack("1B",bit20)[0]#! tempo di riferimento
        timerangeindicator=struct.unpack("1B",bit21)[0]
        if timerangeindicator==10:#! tempo di riferimento per grib estesi a 16 gg >256ore
            record.P2=record.P1*256+record.P2
        ...
        #leggiamo solo le griglie equidistanti
        lenghtsez2=0
        ...
        if flagdata[0]=="0" and flagdata[1]=="0" and flagdata[2]=="0" and flagdata[3]=="0":
            ...
            for N in range(0,record.NI*record.NJ):
                #leggiamo nbit a mutipli di 8 e ne conserviamo il resto nel buffer
                n=(nbit-len(bufferstr))/8
                if math.fmod(nbit-len(bufferstr),8)>0:#bug risolto in release 02
                    n=n+1
                ...
                i=tuplaij[0]
                j=tuplaij[1]
            file.read(lenghtsez4-11-octectreaded)
        else:
...
def test():
    nomefile='GRIBB/20150924_085842_.grb'
    #nomefile='GRIBB/mini2015/GFS20151023095102739.grb'
    #nomefile='GRIBB/20131011_033237.grb'
