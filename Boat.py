...
class Ellissi:
    ...
    def calcolaparametri(self,X,Y):
        ...
        a=self.a
        b=a/self.e
        r1=a*b/((b*math.cos(teta))**2+(a*math.sin(teta))**2)**0.5#eq.polare
        n=0
        while (r-r1)**2>0.0001 and n<10**6:
            n=n+1
            a=a+(r-r1)
            b=a/self.e
            r1=a*b/((b*math.cos(teta))**2+(a*math.sin(teta))**2)**0.5
        self.a=a    
    ...
    def calcolatangente(self,Teta):
        ...
        teta=self.d-Teta
        t=math.atan2(self.e*math.sin(teta),math.cos(teta))#relazione tra parametro t e Teta
        x=self.a*math.cos(t)
